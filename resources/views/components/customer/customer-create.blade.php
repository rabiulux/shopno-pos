    ...
    async function Save() {
        ...
        try {
            ...
            if (response.status === 201 && response.data.status === 'success') {
                successToast(response.data.message || 'Customer created successfully');
                document.getElementById('save-form').reset();
                document.getElementById('modal-close').click();
                await fetchCustomers();
            } else {
                errorToast(response.data.message || 'Failed to create customer');
            }
        } catch (error) {
            hideLoader();
            errorToast(error.response?.data?.message || 'An error occurred while creating customer');
        }   
